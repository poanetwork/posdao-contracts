...
// function findMapLocation(slot, key1, key2) {
//   const slot2 = web3.utils.keccak256(web3.eth.abi.encodeParameters(['uint256','uint256'], [key1, slot]));
...
async function main() {
  ...
  // web3.setProvider('https://rpc.xdaichain.com');
  // console.log(await ValidatorSet.methods.upgradeToAndCall('0xBD93Dd777692DD2B622dc2ed155CB3B654fd75dA', '0x0e9296ca').estimateGas({from: '0x20dcD414A3D6E57e92373871715FB9343c43B1a3'}));
  // return;
  ...
  //   console.log(`  maliciousValidator = ${events[i].returnValues.maliciousValidator}`);
  //   console.log(`  blockNumber = ${events[i].returnValues.blockNumber}`);
  //   console.log('');
  // }
  // return;
...
async function getCurrentValidators(miningAddresses, blockNumber) {
  ...
  poolIds.forEach(poolId => {
    promises.push(new Promise((resolve, reject) => {
      batch.add(ValidatorSet.methods.poolName(poolId).call.request({}, blockNumber, (err, result) => {
        if (err) reject(err);
        else resolve(result);
      }));
    }));
  ...
  await batch.execute();
  const poolNames = await Promise.all(promises);
  ...
  miningAddresses.forEach((miningAddress, index) => {
    ...
    const name = knownNames[miningAddress] || poolNames[index] || stakingAddressShort;
    knownNames[miningAddress] = name;
