...
import "../interfaces/IBlockRewardHbbftTokens.sol";
import "../interfaces/IStakingHbbftTokens.sol";


contract BlockRewardHbbftTokens is BlockRewardHbbftBase, IBlockRewardHbbftTokens {
    ...
    /// @dev Called by the `StakingHbbft.claimReward` function to transfer tokens and native coins
    ...
    function transferReward(uint256 _tokens, uint256 _nativeCoins, address payable _to) external onlyStakingContract {
        if (_tokens != 0) {
            IStakingHbbftTokens stakingContract = IStakingHbbftTokens(msg.sender);
    ...
    function _distributeTokenRewards(
        ...
        IERC677Minting erc677TokenContract = IERC677Minting(
            IStakingHbbftTokens(_stakingContract).erc677TokenContract()
        );
